...
- **DO** `// new` 주석은 **오직 `i18n/messages.ts` 에서 메시지 키를 신규 추가하거나 기존 메시지 텍스트를 수정한 그 라인 끝에만** 부착한다. 목적은 사용자가 "화면 문구" 변경만 diff 에서 빠르게 스캔하기 위함이다.
  - **판단 기준은 하나뿐:** "이 라인이 `i18n/messages.ts` 의 화면 노출 문구(jp/kr) 라인인가?" → 예면 부착, 아니면 **금지**. "새로 추가/수정된 줄이니까", "객체 필드니까", "기존 코드에도 붙어 있으니까" 같은 다른 근거로 **절대 확대 적용하지 않는다.**
  - **DON'T** 코드 파일(`.ts`/`.tsx`)에는 붙이지 않는다 — import, 타입/인터페이스 필드, 함수·상수·변수 선언, 로직·분기, JSX 속성, 코드 안의 객체/매핑 리터럴 **전부 제외**. (i18n 키를 참조하는 `t("...")` 호출부도 코드이므로 제외.)
  - **기존 코드 파일에 남아 있는 `// new`(import·함수·로직 등)는 이 규칙이 확정되기 전의 잔재다. 관례·근거로 삼아 흉내내지 말 것.** 해당 파일을 수정할 때 정리(제거)해도 된다.
  - 이 규칙은 §0 의 "변경 이력 주석 금지"에 대한 **`i18n/messages.ts` 한정 예외**다. 다른 앱·파일에는 적용하지 않는다.
  - 이후 별개 작업으로 `messages.ts` 를 다시 편집할 때 앞선 `// new` 는 정리(제거)해도 되고 유지해도 된다 — 사용자가 리뷰 후 요청한 대로 처리.
