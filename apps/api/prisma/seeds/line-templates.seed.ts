...
const INSIGHT_REMINDER = `📊【本日がインサイト提出日です】数値の登録をお願いいたします 📊
お世話になっております。
ご参加いただいている「{{campaignTitle}}」の投稿から7日が経過し、本日がインサイト提出の目安日となります。素敵なご投稿をいただき、誠にありがとうございます。

キャンペーンの最終精算および成果測定のため、本日中にインサイト資料のご提出をお願いいたします。
...
const INSIGHT_OVERDUE_REMINDER = `⚠️【インサイト未提出のお知らせ】お早めのご提出をお願いいたします ⚠️
お世話になっております。
ご参加いただいている「{{campaignTitle}}」のインサイト提出目安日を1日過ぎておりますが、まだご提出が確認できておりません。

キャンペーンの精算手続きに関わりますので、大変恐れ入りますが、お早めに下記の方法でご提出をお願いいたします。

📝 提出項目のご案内
- 対象インサイト: いいね数・コメント数・シェア数・リポスト数・保存数・閲覧数・リーチ数などの画面スクリーンショットおよび数値入力

※サイト内の【応募履歴 - インサイト提出】より、投稿の成果数値がわかる画面をスクリーンショットしてご登録ください。

ご協力のほどよろしくお願いいたします。

※自動送信のため返信不要ですが、ご不明な点はお気軽にお問い合わせください。
※すでにご提出済みの場合は行き違いにつき、ご容赦ください。
🕐 運営：平日 10:00〜20:00`;
...
const SEED_ROWS: SeedRow[] = [
  ...
  { triggerKey: "SNS_INSIGHT_OVERDUE_REMINDER", enabled: true, body: INSIGHT_OVERDUE_REMINDER },
