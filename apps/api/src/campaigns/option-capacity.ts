import { usesOptionCountSplit } from "@jsure/shared";

type RecruitWithOptions<SubType extends string> = {
  subType: SubType;
  options: { option: string; recruitCount: number | null }[];
};

export type OptionSlot<SubType extends string = string> = {
  subType: SubType;
  option: string;
  recruitCount: number;
};
 ...
 */
export function optionCapacitySlots<SubType extends string>(
  recruits: RecruitWithOptions<SubType>[],
): OptionSlot<SubType>[] {
 ...
 */
export function subTypesWithAllOptionsFull<SubType extends string>(
  recruits: RecruitWithOptions<SubType>[],
  fullOptions: { subType: SubType; option: string }[],
): SubType[] {
  return recruits
    .filter(
      (recruit) =>
