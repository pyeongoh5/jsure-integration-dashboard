...
| D-15 | 당첨자 목록 필터·CSV 위치 | **필터는 서버(SQL) + 커서 페이징 50건. CSV 는 목록과 같은 where 를 쓰는 전용 엔드포인트(`GET /admin/campaigns/:id/winners/export`)가 필터에 걸린 전체를 배송지까지 복호화해 내려주고 감사에 기록.** 이행 처리로 필터에 안 맞게 된 행은 목록에서 제거 | 화면이 전량 로드 후 거르면 데이터가 늘었을 때 "보이는 목록 ≠ 실제 전체"가 되고, CSV 가 조용히 첫 페이지만 담는다(배송 누락으로 직결). where 생성을 `winnerFilterWhere` 한 곳에 두어 목록과 CSV 가 다른 집합을 보지 않게 했다. 배송지 평문은 목록 응답에 섞을 수 없고 반출 자체가 감사 대상이라 엔드포인트 분리가 필연이었다. **트레이드오프**: 필터를 바꿀 때마다 서버 왕복이 생기고, 이행 처리 후 행이 목록에서 사라지는 동작을 운영자가 처음엔 낯설게 느낄 수 있다 | 2026-09-05 |
