...
| 11 | 어드민 통계 확인 · 현물 발송 처리 · 종료 | 어드민 | ○ (Phase 5 완료) |

즉 **11단계 전 과정이 화면으로 닫힌다. 남은 것은 배포(Phase 6)뿐이다.**
...
| 영역 | 내용 |
|------|------|
| 배포 | Neon·Railway·Vercel 프로젝트 미생성 (`DEPLOY.md`) |

어드민 화면(S1~S3)·어드민 API·G0 스파이크는 모두 완료됐다. 상세는 §5 Phase 3~5 와 `DECISIONS.md` §G0 참조.
...
### Phase 5 — S2 통계 탭 + S3 당첨자 관리 ✅ 완료 (2026-09-05)

1. 통계 탭 — 응모·당첨 확정·당일 검증 대기·미이행 종료·게시 실패 카드 + 경품별 잔여 재고
2. 당첨자 목록·필터 — **필터는 서버(SQL), 페이징은 커서 50건**
3. 배송지 열람 다이얼로그(감사 기록) + 발송 완료 처리(`READY → SHIPPED` 만 허용)
4. CSV 내보내기 — 전용 엔드포인트가 **필터에 걸린 전체**를 배송지까지 복호화해 내려주고 감사에 남긴다
5. 완료 기준 충족: **화면만으로 캠페인을 종료하고 현물 발송까지 닫을 수 있다** (전 과정 11단계)

계획 대비 달라진 점: 당첨자 목록 필터가 화면 전량 로드 방식이었는데, 데이터가 늘면 "보이는 목록 ≠ 실제 전체"가 되고 CSV가 조용히 일부만 담는다. 서버 필터 + 커서 페이징으로 바꾸고, CSV는 목록과 같은 where 를 쓰는 별도 엔드포인트(`GET /admin/campaigns/:id/winners/export`)로 분리했다. 이행 처리 후 필터에 더는 맞지 않는 행은 목록에서 빼서 화면·CSV 개수가 어긋나지 않게 한다.

### Phase 6 — 배포 전

G0 스파이크는 **2026-08-28 전 항목 통과**했다 (`DECISIONS.md` §G0). DM 발송이 열려 있음이 확인돼 D-4는 그대로 간다. 남은 것은 `DEPLOY.md` §6 운영 체크리스트다.

G0에서 발견된 `media.write` 스코프 누락은 수정했으나, **기연동 브랜드 계정은 재연동해야 한다** — refresh 로는 스코프가 붙지 않는다. 배포 체크리스트에 포함할 것.
