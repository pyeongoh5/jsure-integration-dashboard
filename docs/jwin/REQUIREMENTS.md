...
| 2 | 어드민 웹 UI | **방향 확정 (D-10) — `@jsure/admin-web`에 J-WIN 모듈로 추가.** 인증 배선(공유 JWT, axios 인스턴스, dev 프록시)과 셸(제품 스위처·`/jwin/*` 라우트)은 완료. 화면 구현은 미착수. **화면 설계와 단계별 실행 계획은 `MVP_PLAN.md` 참조** |
