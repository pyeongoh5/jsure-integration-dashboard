...
그러나 admin-web의 **응모관리(Applicants)** 페이지와 **검토(Drafts)** 페이지에서는 Instagram 응모에서 어떤 형식을 선택했는지 시각적으로 확인할 수 없다.

## 목표

- 응모관리·검토 페이지의 SNS 아이콘 **아래**에 Instagram 응모일 때만 "피드"/"릴스" 라벨을 작은 텍스트로 표시한다.
- 비-Instagram 응모와 `instagramPostType`이 `null`인 과거 데이터에는 라벨을 표시하지 않는다.

## 비목표

- 데이터 모델·DB·응모 생성·캠페인 모델은 변경하지 않는다.
- client-web 응모 UI 변경은 별도 작업이며 본 spec에 포함되지 않는다.
...
- 라벨 매핑 `INSTAGRAM_POST_TYPE_LABEL: Record<InstagramPostType, string> = { FEED: "피드", REELS: "릴스" }`는 `apps/admin-web/src/domains/campaign/types.ts:20`에 이미 존재. 두 페이지에서 재사용한다.
...
| 상황 | 표시 |
|------|------|
| SNS = Instagram, instagramPostType = FEED | 인스타 아이콘 + "피드" |
| SNS = Instagram, instagramPostType = REELS | 인스타 아이콘 + "릴스" |
| SNS = Instagram, instagramPostType = null | 인스타 아이콘만 (과거 데이터 호환) |
| SNS ≠ Instagram | 해당 SNS 아이콘만 |
...
- 비-Instagram 응모(X 등)와 `instagramPostType`이 `null`인 인스타 응모는 라벨 없이 아이콘만 보인다.
- 검토 페이지에서도 동일한 표시 규칙이 적용된다.
- `pnpm typecheck`가 통과한다(기존 사전 존재 오류 `Drafts/index.tsx:85` showStageFilter 제외).
- 시각적으로 기존 행 높이가 라벨 없는 케이스에서는 변하지 않는다(라벨이 있을 때만 약간 늘어남 — 허용).
