...
**DM 미리보기**: `dmTemplate` 아래에 치환 예시를 **실시간 렌더**한다. 지원 플레이스홀더는 `{{CODE}}` `{{PRIZE_NAME}}` `{{USERNAME}}` `{{BRAND_NAME}}`. 코드가 실제로 들어갈 자리를 눈으로 확인하지 않으면 `{{CODE}}`가 빠진 DM을 발송하는 사고가 난다.

**저장 차단**: CODE 경품이 하나라도 있는데 `dmTemplate`에 `{{CODE}}`가 없으면 **저장을 막는다**. 코드 없는 DM이 나가면 당첨자가 경품을 받을 수 없다.
